# AAP-Splendor128 スプライトシートジェネレーター

AAP-Splendor128カラーパレット対応のスプライトシートジェネレーターです。ゲーム開発やピクセルアート制作において、複数のスプライト画像を1枚のシート画像にまとめて出力できます。
...
### AAP-Splendor128カラーパレット
- 128色の専用カラーパレットを搭載
- 各ピクセルは0〜128の数値で指定
  - `0`: 透明
  - `1-128`: AAP-Splendor128パレットの各色に対応
...
- **最大スプライト数**: 256個
- **カラーパレット**: AAP-Splendor128（128色 + 透明）
- **出力形式**: PNG
...
### 最新版
- AAP-Splendor128（128色）カラーパレットに対応
  - Resurrect64（64色）から移行
  - より豊富な色数でピクセルアート制作が可能に
...
- **AAP-Splendor128 パレット**: [AAP-Splendor128](https://lospec.com/palette-list/aap-splendor128) by Adigun A. Polack
